
///<reference path="application/files/LocalStorage.ts" />

///<reference path="Config.ts" />

///<reference path="directives/DropZone.ts" />
///<reference path="directives/Panel.ts" />
///<reference path="directives/Part.ts" />
///<reference path="directives/Buttons.ts" />
///<reference path="directives/Icon.ts" />
///<reference path="directives/Dialog.ts" />

///<reference path="controllers/AppController.ts" />
///<reference path="controllers/ImagesController.ts" />
///<reference path="controllers/DownloadController.ts" />
///<reference path="controllers/MenuController.ts" />
///<reference path="controllers/ImageEditController.ts" />
///<reference path="controllers/ImageRenameController.ts" />
///<reference path="controllers/ImageDeleteController.ts" />


